...
async function openCreateDocumentModal() {
    ...
    document.getElementById('docTagsContainer').innerHTML = 'Загрузка тегов...';
    document.getElementById('docTagsSelect').innerHTML = '';
    document.getElementById('docCategory').innerHTML = '<option value="">Сначала выберите курс</option>';
...
async function loadCoursesForModal() {
    const select = document.getElementById('docCourse');
    try {
        const response = await fetch('/admin/documents/courses');
        const result = await response.json();
        if (result.success && result.data) {
            ...
            select.innerHTML = '<option value="">Не удалось загрузить курсы</option>';
        }
    } catch (error) {
...
async function loadAndRenderTags() {
    const container = document.getElementById('docTagsContainer');
    const select = document.getElementById('docTagsSelect');
    ...
    try {
        ...
        if (tags && tags.length > 0) {
            tags.forEach(tag => {
                ...
                badge.className = 'badge rounded-pill text-bg-secondary p-2 m-1';
                ...
                badge.addEventListener('click', () => {
                    ...
                    badge.classList.toggle('text-bg-primary');
                    badge.classList.toggle('text-bg-secondary');

                    const option = select.querySelector(`option[value="${tag.name}"]`);
            ...
            container.textContent = 'Теги не найдены.';
        }
    } catch (error) {
        container.textContent = 'Ошибка загрузки тегов.';
    }
}
...
async function handleCreateDocumentSubmit(event) {
    ...
    // Manually build FormData to ensure correctness
    formData.append('name', form.querySelector('#docName').value);
    formData.append('categoryId', form.querySelector('#docCategory').value);
    formData.append('textContent', form.querySelector('#docTextContent').value);

    const imageFile = form.querySelector('#docImageFile').files[0];
    if (imageFile) {
        formData.append('file', imageFile);
    }

    // Get all selected tags from the hidden select
    const selectedTags = document.querySelectorAll('#docTagsSelect option:checked');
